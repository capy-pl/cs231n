...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        dout, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(
            dout, cache_scores)
        if self.cell_type == 'lstm':
            dout, dh, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(
                dout, cache_lstm)
        ...
        dscore, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(
            dout, score_cache)
        drnn, dh0_rnn, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(
            dscore, rnn_cache)
        grads['W_embed'] = word_embedding_backward(drnn, word_cache)
        dx_feature, grads['W_proj'], grads['b_proj'] = affine_backward(
            dh0_rnn, feat_cache)
